...
Imports PageItemColxnServicesLibr
Imports System.Configuration


Class Window1

    Const baseaddressPgItemColxnSvc = "baseaddressPgItemColxnSvc"

    Private Shared ThisInstance As Window1
    Public Sub IamLoaded() Handles Me.Loaded
        ThisInstance = Me
        CommandSupport.MyBaseAddress = ConfigurationManager.AppSettings(baseaddressPgItemColxnSvc)
        PgItemsColxnSvcMonitor = New PageItemColxnServicesLibr.ServiceMonitor
        'SetUpPgItemsColxnSvcHost()
        Try
            CmdSvcMonitor = New PageItemColxnServicesLibr.CommandSvcMonitor
            SetUpCommandSvcHost()
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles Button1.Click
        ...
        If MyPgItemsColxnSvcHost IsNot Nothing Then
            If MyPgItemsColxnSvcHost.State <> CommunicationState.Opened AndAlso MyPgItemsColxnSvcHost.State <> CommunicationState.Opening Then
                SetUpPgItemsColxnSvcHost()
            End If
        Else
            SetUpPgItemsColxnSvcHost()
    ...
    Private MyPgItemsColxnSvcHost As CustomSvcHost

    'Private SvcHostDxnry As New Dictionary(Of String, ServiceHost)

    Private Sub SetUpPgItemsColxnSvcHost()
        MyPgItemsColxnSvcHost = New PageItemColxnServicesLibr.CustomSvcHost(GetType(PageItemColxnServicesLibr.PgItemColxnSvc))
        AddHandler MyPgItemsColxnSvcHost.Opened, AddressOf UpdateStateTextBlock
        AddHandler MyPgItemsColxnSvcHost.Faulted, AddressOf UpdateStateTextBlock
        AddHandler MyPgItemsColxnSvcHost.Closing, AddressOf UpdateStateTextBlock
        AddHandler MyPgItemsColxnSvcHost.Closed, AddressOf UpdateStateTextBlock
        MyPgItemsColxnSvcHost.Open()

        If MyPgItemsColxnSvcHost IsNot Nothing Then
            Dim tb As New TextBlock
            tb.Text = MyPgItemsColxnSvcHost.Description.Name & " SetUp @ " & DateTime.Now.ToLongTimeString
            tb.Margin = New Thickness(2.0)
            Me.ServiceListbox.Items.Add(tb)
            Dim tb1 As New TextBlock
            tb1.Margin = New Thickness(2.0)
            tb1.Text = MyPgItemsColxnSvcHost.Description.Endpoints.FirstOrDefault.ListenUri.OriginalString & " " & DateTime.Now.ToLongTimeString
    ...
    Private Sub UpdateStateTextBlock()
        If MyPgItemsColxnSvcHost IsNot Nothing Then
            ...
            Dim tb As New TextBlock
            tb.Margin = New Thickness(2.0)
            tb.Text = MyPgItemsColxnSvcHost.State.ToString & " " & DateTime.Now.ToShortTimeString
            lbi.Content = tb
            Me.StateListbox.Items.Add(lbi)
            Me.StateListbox.SelectedItem = lbi
            Me.StateListbox.ScrollIntoView(lbi)
        End If
        If MyPgItemsColxnSvcHost.State = CommunicationState.Faulted Then
            SetUpPgItemsColxnSvcHost()
        End If
    End Sub
    ...
    Private Sub WindowClosing() Handles Me.Closing
        If MyPgItemsColxnSvcHost IsNot Nothing Then
            If MyPgItemsColxnSvcHost.State <> CommunicationState.Closing AndAlso MyPgItemsColxnSvcHost.State <> CommunicationState.Closed Then
                MyPgItemsColxnSvcHost.Close()
                MyPgItemsColxnSvcHost = Nothing
            End If
        End If
    End Sub
    ...
    Private Shared WithEvents PgItemsColxnSvcMonitor As PageItemColxnServicesLibr.ServiceMonitor
    Private Shared Sub PgItemsColxnSvcMonitorPropertyChanged() Handles PgItemsColxnSvcMonitor.PropertyChanged
        ThisInstance.OperationsMonitorListBox.ItemsSource = Nothing
        ThisInstance.OperationsMonitorListBox.ItemsSource = PageItemColxnServicesLibr.ServiceMonitor.ServiceCalls
    ...
    Private Sub Button3_Click_1(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs)
        ...
        Me.OperationsMonitorListBox.ItemsSource = PageItemColxnServicesLibr.ServiceMonitor.ServiceCalls
...
#Region "Command Svc Stuff"
    ...
    Private Shared WithEvents CmdSvcMonitor As PageItemColxnServicesLibr.CommandSvcMonitor
    Private Shared Sub ServiceMonitorPropertyChanged() Handles CmdSvcMonitor.PropertyChanged
        ...
        ThisInstance.CmdWindow.CommandsListBox.ItemsSource = PageItemColxnServicesLibr.CommandSvcMonitor.CommandSvcCalls
        ...
        ThisInstance.CommandHandler(PageItemColxnServicesLibr.CommandSvcMonitor.CommandIssued)
    End Sub
...
''' <summary>
''' CommandSupport for PgItemColxnServices Container
''' </summary>
''' <remarks></remarks>
Public Class CommandSupport
    ...
    Public Shared Sub StartCmd(ByVal _cmdPkg As CmdSvcClassLibrary.CommandPackage) 'ByVal _EptSuffix As String)
        ...
        If _EptSuffix = Nothing Then
            MessageBox.Show("EptSuffix is nothing...")
        End If
        If SvcHostDxnry.ContainsKey(_EptSuffix) Then 'TryGetValue(_queueUri, xxxsvchost) Then
            If SvcHostDxnry.Item(_EptSuffix).State <> CommunicationState.Opening AndAlso SvcHostDxnry.Item(_EptSuffix).State <> CommunicationState.Opened Then
                Try
                    Dim newPSHost = New CustomSvcHost(GetType(PgItemColxnSvc))
                    ...
                    Dim endpt = newPSHost.AddServiceEndpoint(GetType(IPgItemColxnSvc), _
                ...
                Catch ex As Exception
                    MessageBox.Show("PgItemColxnSvcContainer.Window1.StartCmd encountered error..." & ex.Message)
                End Try
            End If
        Else
            Try
                ...
                Dim endpt = newPSHost.AddServiceEndpoint(GetType(IPgItemColxnSvc), New BasicHttpBinding("binding1"), _
                                                         MyBaseAddress & _EptSuffix)
                endpt.Name = newPSHost.Description.Name '& "altered"
    ...
    Public Shared Sub StopCmd(ByVal _EptSuffix As String)
        ...
        If SvcHostDxnry.ContainsKey(_EptSuffix) Then 'TryGetValue(_queueUri, xxxsvchost) Then
            Try
                ...
                End If
            Catch ex As Exception
                MessageBox.Show("PgItemColxnSvcContainer.Window1.StopCmd encountered error..." & ex.Message)
            End Try
    ...
    Private Shared Sub SvcFaulted_Handler(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If SvcHostDxnry.TryGetValue(_EptSuffix, svchost) Then
            Try
                'svchost = New CustomSvcHost(GetType(PgItemColxnSvc))
                ...
                'Dim endpt = svchost.AddServiceEndpoint(GetType(IPgItemColxnSvc), _
                '                                       New BasicHttpBinding("binding1"), MyBaseAddress & _EptSuffix)
                'endpt.Name = svchost.Description.Name '& "altered"
                'Dim tcpendpt = svchost.AddServiceEndpoint(GetType(IPgItemColxnSvc), New NetTcpBinding(), MyBaseAddress & _EptSuffix)
            ...
            Catch ex As Exception
                MessageBox.Show("PgItemColxnSvcContainer.Window1.SvcFaulterHandler encountered error..." & ex.Message)
            End Try
        End If
    End Sub
End Class